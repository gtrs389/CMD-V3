...
-- Migration 040: troca do dominio do sistema (de volta)
--
-- Execute no SQL Editor do Supabase depois de 039_demo_segunda_camada.sql.
...
-- POR QUE ELA EXISTE
--
-- O dominio do sistema mudou de novo, desta vez na volta — a
-- `037_troca_de_dominio.sql` tinha feito o caminho contrario:
--
--   7061696e656c2061646d.appdemo.sbs  ->  7061696e656c2061646d.convitetimebezerra.com
--   painel.appdemo.sbs                ->  painel.convitetimebezerra.com
--   www.appdemo.sbs                   ->  www.convitetimebezerra.com
--
...
update public.cmd_settings
   set public_link_origin  = replace(public_link_origin,  'appdemo.sbs', 'convitetimebezerra.com'),
       public_redirect_url = replace(public_redirect_url, 'appdemo.sbs', 'convitetimebezerra.com'),
       updated_at          = now()
 where public_link_origin  like '%appdemo.sbs%'
    or public_redirect_url like '%appdemo.sbs%';
