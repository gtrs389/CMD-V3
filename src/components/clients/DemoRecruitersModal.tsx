...
export function DemoRecruitersModal({
  ...
  const valor = editado ?? (servidor === null ? '' : String(servidor));

  useEffect(() => {
    let atual = true;

    api<{ recruiters: number }>(`/api/clients/demo/${client.id}/recrutadores`)
      .then((resposta) => {
        if (atual) setServidor(resposta.recruiters);
      })
      .catch(() => {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      const resposta = await api<{ recruiters: number; moved: number }>(
        `/api/clients/demo/${client.id}/recrutadores`,
        { method: 'PATCH', body: { recruiters: Number(valor) || 0 } },
      );

      toast.success(
        resposta.recruiters === 0
          ? 'Segunda camada desfeita: todos os cadastros voltaram para o administrador.'
          : `${resposta.recruiters} pessoa(s) recrutando, com ${resposta.moved} cadastro(s) no nome delas.`,
      );
      onChanged?.();
      onClose();
    } catch (falha) {
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-ink-500">
          ...
          deste Time DEMO aparecem recrutando — é o que enche o{' '}
          <strong className="font-semibold text-ink-700">Ranking de cadastros equipe</strong>.
        </p>

        <Field
          id="demo-recrutadores-total"
          label="Quantas pessoas recrutam"
          help={`De 0 a ${DEMO_RECRUITERS_MAX}. Zero desfaz e devolve tudo ao administrador.`}
        >
          {carregando ? (
            <span className="flex h-11 items-center gap-2 text-sm text-ink-500">
              <Spinner className="size-4" /> Carregando o valor de hoje…
            </span>
          ) : (
            <Input
              id="demo-recrutadores-total"
        ...
        <p className="rounded-control border border-line bg-ink-50 p-3 text-[0.8125rem] text-ink-600">
          Elas aparecem na hierarquia e no ranking, mas <strong>não entram no painel</strong>: os
          telefones de um Time DEMO são fictícios, e o acesso delas nasce desligado.
        </p>
      </div>
    </Modal>
  );
